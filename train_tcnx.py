...
def compute_metrics_phys(model, loader, device, y_mu_1d: np.ndarray, y_sd_1d: np.ndarray):
    """
    计算按通道的 MAE/RMSE/R2（物理单位），对整个 loader 聚合。
    返回: mae(C,), rmse(C,), r2(C,)
    """
    model.eval()
    C = int(len(y_sd_1d))
...
def main():
    ...
    ap.add_argument('--shape_only_loss', action='store_true',
                    help='逐窗z-score仅比较形状（去直流&尺度），常与 shift_tolerance 配合。')
    ap.add_argument('--shift_tolerance', type=int, default=0,
                    help='损失中允许 ±S 帧时移搜索，0禁用。建议先试 2~4。')
    args = ap.parse_args()
    ...
    assert len(files) > 0, "数据目录下未找到 Excel。"
    ...
    gap = int(np.ceil(args.seq_len / max(1, args.stride))) if use_val else 0  # 仅有验证集时才留gap
    ...
    for f in files:
        ...
        if use_val:
            ...
            n_val = min(max(n_val, 1), n)  # 至少1，至多n
    ...
    assert len(train_subsets) > 0, "训练集为空，请检查数据或参数。"
    if use_val:
        assert len(val_subsets) > 0, "验证集为空，请调小 val_ratio 或 gap。"
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if use_val:
            ...
            print(f'Epoch {epoch:03d} | train {tr:.6f} | '
                  f'lr {optimizer.param_groups[0]["lr"]:.2e}')
            cur_metric = tr  # 无验证时，以训练损失作为最优判据
