# test_tcn.py
# 用训练好的 TCN 模型测试新 Excel 数据，并输出预测值 + metrics
...
# ================== 路径设置 ==================
TEST_DATA_DIR = Path(r"F:/chengxu/DM-Tac/train_data_04")
...
def find_excel_files(data_dir: Path):
    ...
    if len(files) == 0:
        raise FileNotFoundError(f"没有在 {data_dir} 找到 Excel/CSV 文件")
    return files
...
def predict_one_file(file_path: Path, model, device, x_mu, x_sd, y_mu, y_sd):
    """
    对单个 Excel/CSV 做滑窗预测，并把重叠窗口预测平均回原始逐帧序列。
    输出:
        df_out: 原始行级别 target + pred
        metrics_rows: 每个通道一行 metrics
    """
    ...
    if missing_inputs:
        raise ValueError(f"{file_path.name} 缺少输入列: {missing_inputs}")

    if missing_outputs:
        raise ValueError(f"{file_path.name} 缺少最后三列真值: {missing_outputs}")
    ...
    if len(df_use) < SEQ_LEN:
        raise ValueError(f"{file_path.name} 有效行数 {len(df_use)} 小于 SEQ_LEN={SEQ_LEN}")
...
def main():
    ...
    print("\n完成")
    print("每个文件预测结果:", OUT_DIR)
    print("每文件 metrics:", metrics_csv)
    print("总体 metrics:", metrics_all_csv)
    print("总体 target/pred npz:", out_npz)
